...
const EVIDENCE_CLASS_WEIGHTS: Record<EvidenceClass, number> = {
  OBSERVED: 1,
  INFERRED: 0.75,
  THEORETICAL: 0.4,
};

export const DEFAULT_DIMENSION_HALF_LIFE_HOURS: Record<TrustDimension, number> = {
...
export function computeDimensionScores(
  ...
  evidenceByDimension: Partial<Record<TrustDimension, EvidenceSource[]>>,
...
export function evidenceClassBreakdown(evidence: EvidenceSource[]): EvidenceClassBreakdown {
  return evidence.reduce<EvidenceClassBreakdown>(
    (breakdown, source) => {
      if (source.evidence_class === 'OBSERVED') breakdown.observed += 1;
      if (source.evidence_class === 'INFERRED') breakdown.inferred += 1;
      if (source.evidence_class === 'THEORETICAL') breakdown.theoretical += 1;
      return breakdown;
    },
...
export function buildTemporalDecay(
  dimension: TrustDimension,
  evidence: EvidenceSource[],
  now: string,
  decayFunction?: TemporalDecayFunction
): TemporalDecay {
  const evidenceTimes = evidence.map(evidenceTime).filter((value): value is string => Boolean(value));
...
export function buildScoreBreakdown(
  ...
  const rawByClass: EvidenceClassBreakdown = { observed: 0, inferred: 0, theoretical: 0 };
  const decayedByClass: EvidenceClassBreakdown = { observed: 0, inferred: 0, theoretical: 0 };
  const ages = evidence.map((source) => evidenceAgeHours(source, now)).filter((age) => age !== null) as number[];
...
export function decayMultiplier(decayFunction: TemporalDecayFunction, ageHours: number): number {
...
export function defaultDecayFunction(dimension: TrustDimension): TemporalDecayFunction {
  return {
    model: 'half-life',
    half_life_hours: DEFAULT_DIMENSION_HALF_LIFE_HOURS[dimension],
    baseline: DEFAULT_DECAY_BASELINE,
  };
}

function evidenceTime(source: EvidenceSource): string | undefined {
  return source.observed_at ?? source.timestamp;
}

function evidenceAgeHours(source: EvidenceSource, now: string): number | null {
...
function evidenceClassKey(evidenceClass: EvidenceClass): keyof EvidenceClassBreakdown {
  if (evidenceClass === 'OBSERVED') return 'observed';
  if (evidenceClass === 'INFERRED') return 'inferred';
  return 'theoretical';
}
